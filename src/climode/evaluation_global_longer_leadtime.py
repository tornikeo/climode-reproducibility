...
Test_loader = DataLoader(Final_test_data[2:],batch_size=6,shuffle=False)
time_loader = DataLoader(time_steps[2:],batch_size=6,shuffle=False)
time_idx_steps = torch.tensor([i for i in range(365)]).view(-1,1)
time_idx = DataLoader(time_idx_steps[2:],batch_size=6,shuffle=False,pin_memory=False)
...
model = torch.load(str(cwd) + "/Models/ClimODE_global_euler_0_model_294.pt",map_location=torch.device('cpu')).to(device)
print(model)
...
Truth  = []
lead_time = 5
Lead_RMSD_arr = {"z":[[] for _ in range(lead_time)],"t":[[] for _ in range(lead_time)],"t2m":[[] for _ in range(lead_time)],"u10":[[] for _ in range(lead_time)],"v10":[[] for _ in range(lead_time)]}
Lead_ACC = {"z":[[] for _ in range(lead_time)],"t":[[] for _ in range(lead_time)],"t2m":[[] for _ in range(lead_time)],"u10":[[] for _ in range(7)],"v10":[[] for _ in range(lead_time)]}
Lead_CRPS = {"z":[[] for _ in range(lead_time)],"t":[[] for _ in range(lead_time)],"t2m":[[] for _ in range(lead_time)],"u10":[[] for _ in range(7)],"v10":[[] for _ in range(lead_time)]}
...
for t_idx in range(args.batch_size - 1):
    for idx, lev in enumerate(levels):
        print("Lead Time ",(t_idx+1)*24, "hours ","| Observable ",lev, "| Mean RMSD ", np.mean(Lead_RMSD_arr[lev][t_idx]), "| Std RMSD ", np.std(Lead_RMSD_arr[lev][t_idx]))
        print("Lead Time ",(t_idx+1)*24, "hours ","| Observable ",lev, "| Mean ACC ", np.mean(Lead_ACC[lev][t_idx]), "| Std ACC ", np.std(Lead_ACC[lev][t_idx]))
        print("Lead Time ",(t_idx+1)*24, "hours ","| Observable ",lev, "| Mean CRPS ", np.mean(Lead_CRPS[lev][t_idx]), "| Std CRPS ", np.std(Lead_CRPS[lev][t_idx]))
