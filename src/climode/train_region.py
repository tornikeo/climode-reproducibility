...
vel_train, vel_val = load_velocity(
    [
        "train_10year_2day_mm_" + str(args.region),
        "val_10year_2day_mm_" + str(args.region),
    ]
)
print(
...
for epoch in range(args.niters):
    ...
    for entry, (time_steps, batch) in enumerate(zip(time_loader, Val_loader)):
        data = (
            batch[0].to(device).view(1, 1, len(paths_to_data) * (args.scale + 1), H, W)
        )
        past_sample = (
            vel_val[entry]
