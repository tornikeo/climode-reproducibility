...
for idx, data in enumerate(paths_to_data):
    Train_data, Val_data, Test_data, time_steps, lat, lon, mean, std, time_stamp = (
        get_train_test_data_batched_regional(
            data,
            train_time_scale,
            val_time_scale,
            test_time_scale,
...
# breakpoint()
const_channels_info, lat_map, lon_map = add_constant_info_region(
    const_info_path, args.region, H, W
)
...
vel_test = torch.from_numpy(
    np.load("test_10year_2day_mm_" + str(args.region) + "_vel.npy")
)
model = torch.load(
    str(cwd) + "Put_your_model_path", map_location=torch.device("cpu")
).to(device)
print(model)
...
for entry, (time_steps, batch) in enumerate(zip(time_loader, Test_loader)):
    ...
    for yr in range(2):
        for t_step in range(1, len(time_steps), 1):
            evaluate_rmsd = evaluation_rmsd_mm_region(
                mean_pred[t_step, yr, :, :, :],
                batch[t_step, yr, :, :, :],
                lat,
                lon,
            ...
            evaluate_acc = evaluation_acc_mm_region(
                mean_pred[t_step, yr, :, :, :],
                batch[t_step, yr, :, :, :],
                lat,
                lon,
